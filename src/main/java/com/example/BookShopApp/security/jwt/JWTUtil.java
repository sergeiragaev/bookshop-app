...
public class JWTUtil {
    ...
    private Claims extractAllClaims(String token) {
        try {
            ...
            JWTBlackListEntity jwtBlackListEntity = jwtRepository.findJWTBlackListEntityByToken(token);
            if (jwtBlackListEntity == null) {
                jwtBlackListEntity = new JWTBlackListEntity();
                jwtBlackListEntity.setToken(token);
                jwtRepository.save(jwtBlackListEntity);
            }
            throw new BookstoreJwtException(e.getLocalizedMessage());
        }
    }
    ...
    public Boolean validateToken(String token, UserDetails userDetails) {
        ...
        return (username.equals(userDetails.getUsername()) && !isTokenExpired(token)
                && jwtRepository.findJWTBlackListEntityByToken(token) == null);
    }
}
